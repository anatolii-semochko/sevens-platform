...
import { CHAIN_DERIVATION_PATH } from '@react/components/wallet/scripts/crypto'
...
export const STORAGE_KEY = 'sevens_wallets'
...
const readStored = async (): Promise<EncryptedAddress[]> => {
    if (hasChromeStorage) {
        return new Promise((resolve) =>
            chromeApi.storage.local.get([STORAGE_KEY], (res: Record<string, EncryptedAddress[]>) =>
...
const writeStored = async (wallets: EncryptedAddress[]): Promise<void> => {
    if (hasChromeStorage) {
        return new Promise((resolve) => chromeApi.storage.local.set({ [STORAGE_KEY]: wallets }, () => resolve()))
    }
...
export const reloadAllWallets = async (): Promise<EncryptedAddress[]> => {
    const base = await readStored()
...
export const addWallet = async (
    ...
    const newAddr: EncryptedAddress = {
        ...
        mnemonicEnc: CryptoJS.AES.encrypt(mnemonic, password).toString(),
    }

    const existing = await readStored()
    const arr = [...existing, newAddr]
    await writeStored(arr)
...
export const removeWallet = async (publicKey: string): Promise<void> => {
    const currentWalletsList = await readStored()
    const newWalletsList = currentWalletsList.filter(
        wallet => wallet.publicKey !== publicKey
    )
    await writeStored(newWalletsList)
}
...
export const renameWallet = async (
    ...
    const walletsList = await readStored()
    walletsList.map((wallet: EncryptedAddress) => {
        if (wallet.publicKey === publicKey) {
            wallet.name = walletName
        }
    })
    await writeStored(walletsList)
}
